...
| **三标签视图切换** | 使用 `Outline` / `Horizontal` / `Vertical` 标签切换视图；`Horizontal` 为横向层级布局，`Vertical` 为向上展开布局 |
...
| **图形视图交互**           | 单击节点在 Peek View 中预览（preserveFocus）、双击在编辑器中打开、Ctrl+点击展开/折叠子节点、滚轮缩放、拖拽平移；展开状态不再显示右侧 `+/-` 徽标；等待动画显示在节点延伸方向（`Horizontal` 右侧、`Vertical` 上方）     |
