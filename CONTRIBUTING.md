...
Map View 提供基于 HTML5 Canvas 2D 的图形视图，通过 `Outline` / `Graph` 标签切换。在 `Graph` 模式下可选择 `Right` / `Left` / `Up` / `Down` 四个方向。布局使用 BFS 层级算法并根据方向进行坐标映射；连边为 Bezier 曲线并带箭头。节点样式为：普通节点圆角矩形、函数声明节点直角梯形（`Left` 方向自动镜像）；通过符号名前的**彩色 Emoji 图标**区分类型（如 💿 Function、📀 Method、📱 Class、🔗 Interface、🔷 Variable、💲 Struct 等），图标颜色与节点边框颜色一致，颜色来源于当前主题的 `--peek-kind-*` CSS 变量。函数参数自动去除，保持标签简洁。支持滚轮缩放（0.2x–5x）、鼠标拖拽平移、ResizeObserver 自动重布局。
