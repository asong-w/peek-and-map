...
Map View 提供基于 HTML5 Canvas 2D 的图形视图，通过「View」按钮在树形列表和图形之间切换。图形使用 BFS 层级布局算法，从左到右排列，Bezier 曲线连边并带箭头。节点统一为圆角矩形，通过符号名前的**彩色图标前缀**区分类型（如 `ƒ` Function=蓝色、`⊛` Method=紫色、`◆` Class=琥珀色、`◇` Interface=青色等），图标颜色与节点边框颜色一致。函数参数自动去除，保持标签简洁。支持滚轮缩放（0.2x–5x）、鼠标拖拽平移、ResizeObserver 自动重布局。
